...
class ScoutCaveHighAction(BaseAction):
    ...
    def _ensure_in_world(self, image: np.ndarray) -> bool:
        ...
        enter_matches = self._city_matcher.match(roi_image, template_name="enter_city_icon", threshold=0.80)
        if enter_matches:
            logger.debug("[ScoutCaveHigh] Already in world view")
            return True
        ...
        if in_city_matches:
            ...
            logger.info(f"[ScoutCaveHigh] Switching to world map at ({cx}, {cy})")
            self.state_machine.pc_input.tap(cx, cy)
            time.sleep(random.uniform(1.0, 3.0))
            return True

        logger.warning("[ScoutCaveHigh] Could not determine city/world state")
        return False
    ...
    def _ensure_in_city(self, image: np.ndarray) -> bool:
        ...
        in_city_matches = self._city_matcher.match(roi_image, template_name="in_city_icon", threshold=0.80)
        if in_city_matches:
            logger.debug("[ScoutCaveHigh] Already in city view")
            return True
        ...
        if enter_matches:
            ...
            cx = roi_x1 + random.randint(bx1, max(bx1, bx2 - 1))
            cy = roi_y1 + random.randint(by1, max(by1, by2 - 1))
            logger.info(f"[ScoutCaveHigh] Switching to city view at ({cx}, {cy})")
            self.state_machine.pc_input.tap(cx, cy)
            time.sleep(random.uniform(1.0, 3.0))
            return True
    ...
    def execute(self) -> bool:
        ...
        cfx, cfy = self._random_point_in_bbox(find_btn.bbox)
